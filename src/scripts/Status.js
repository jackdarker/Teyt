...
class Stat {
    ...
    formatMsgStatChange(attr,_new,_old) {
        if((_new-_old)>0) { 
            return('<statup>'+attr.id+"off"+this.parent.parent.name+" regenerated by "+(_new-_old).toFixed(1).toString()+"</statup></br>");
        } else if((_new-_old)<0) {
            return('<statdown>'+attr.id+"off"+this.parent.parent.name+" decreased by "+(_new-_old).toFixed(1).toString()+"</statdown></br>");
        }
        return("");
    };
    updateModifier() {};
